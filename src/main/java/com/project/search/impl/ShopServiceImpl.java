...
import com.project.search.dao.ShopModelMapper;
...
public class ShopServiceImpl implements ShopService {
    ...
    public ShopModel create(ShopModel shopModel) throws BusinessException {
        ...
        //1. check whether the Seller exists or not
        SellerModel sellerModel = sellerService.get(shopModel.getSellerId());
        if(sellerModel == null){
            throw new BusinessException(BusinessErrorEnum.PARAMETER_VALIDATION_ERROR, "Seller doesn't exist");
        }

        //2. check whether the Seller is disabled or not
        if(sellerModel.getDisabledFlag().intValue() == 1 ){
            throw new BusinessException(BusinessErrorEnum.PARAMETER_VALIDATION_ERROR, "Seller is disabled");
        }

        //3. check whether the Category exists or not
        CategoryModel categoryModel = categoryService.get(shopModel.getCategoryId());
        if(categoryModel == null){
            throw new BusinessException(BusinessErrorEnum.PARAMETER_VALIDATION_ERROR, "Category doesn't exist");
        }
    ...
    public Map<String, Object> searchES(BigDecimal longitude, BigDecimal latitude, String keyword, Integer orderby,
        ...
        /* Simple Version for searching in ES
